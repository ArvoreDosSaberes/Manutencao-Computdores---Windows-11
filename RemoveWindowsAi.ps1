...
If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator')) {
    ...
    $arglist = "-NoProfile -ExecutionPolicy Bypass -C `"& ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/ArvoreDosSaberes/Manutencao-Computdores---Windows-11/main/RemoveWindowsAi.ps1')))"
    ...
    if (!$revert) {
        ...
        if (!$package) {
            ...
            if ((Test-Path "$PSScriptRoot\RemoveWindowsAIPackage\amd64") -and (Test-Path "$PSScriptRoot\RemoveWindowsAIPackage\arm64")) {
                ...
                try {
                    Invoke-WebRequest -Uri "https://github.com/ArvoreDosSaberes/Manutencao-Computdores---Windows-11/raw/refs/heads/main/RemoveWindowsAIPackage/$arch/ZoicwareRemoveWindowsAI-$($arch)1.0.0.0.cab" -OutFile "$($tempDir)ZoicwareRemoveWindowsAI-$($arch)1.0.0.0.cab" -UseBasicParsing -ErrorAction Stop
                }
                catch {
                    Write-Status -msg "Unable to Download Package at: https://github.com/ArvoreDosSaberes/Manutencao-Computdores---Windows-11/raw/refs/heads/main/RemoveWindowsAIPackage/$arch/ZoicwareRemoveWindowsAI-$($arch)1.0.0.0.cab" -errorOutput
                    return
                }
...
function Update-Cleanup-Check {
    ...
    if (!$revert) {
        ...
        try {
            ...
            $scriptContent = Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/ArvoreDosSaberes/Manutencao-Computdores---Windows-11/main/RemoveAI-UpdateCleanup.ps1' -UseBasicParsing -ErrorAction Stop | Select-Object Content
        }
        catch {
...
function install-classicapps {
    ...
    if (Test-Path "$PSScriptroot\ClassicApps") {
        ...
        if (!(Test-Path "$($tempDir)ClassicApps")) {
            ...
            $url = 'https://github.com/ArvoreDosSaberes/Manutencao-Computdores---Windows-11/archive/refs/heads/main.zip'
            try {
            ...
            $sourceDir = "$($tempDir)Manutencao-Computdores---Windows-11-main\ClassicApps"
            $destDir = "$($tempDir)ClassicApps"
            Copy-Item -Path $sourceDir -Destination $destDir -Recurse -Force
            Remove-Item "$($tempDir)Manutencao-Computdores---Windows-11-main" -Recurse -Force -ErrorAction SilentlyContinue
...
'@
    ...
    $githubButton.Add_Click({
            Start-Process 'https://github.com/ArvoreDosSaberes/Manutencao-Computdores---Windows-11'
        })
